...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const autoRotate = () => {
      if (
        ...
        rotationRef.current.y += autoRotateSpeedRef.current * delta * 60;
        applyTransform(rotationRef.current.x, rotationRef.current.y);
      } else {
  ...
  const cssStyles = `
    ...
    @keyframes subtleWiggle {
      ...
      50% {
        transform: rotateZ(-0.3deg) scale(0.99);
      }
      75% {
        transform: rotateZ(0.4deg) scale(1.005);
      }
    }
    ...
    .item__image {
      ...
      animation-delay: calc(var(--offset-x, 0) * 0.05s + var(--offset-y, 0) * 0.03s);
      box-shadow: 0 0 0 rgba(255, 109, 0, 0);
    }
    .item__image:hover {
      box-shadow: 0 0 20px 4px rgba(255, 109, 0, 0.8), 
                  0 0 40px 8px rgba(255, 109, 0, 0.4),
                  inset 0 0 20px 2px rgba(255, 109, 0, 0.3);
    }
    .item__image--reference {
      position: absolute;
      inset: 10px;
      ...
      >
        <main
          ...
          onMouseEnter={() => {
            ...
            const slowDown = () => {
              if (autoRotateSpeedRef.current > 0.04) {
                autoRotateSpeedRef.current *= 0.93;
                requestAnimationFrame(slowDown);
              } else {
                autoRotateSpeedRef.current = 0.03;
              }
            };
            slowDown();
          }}
          onMouseLeave={() => {
            ...
            const speedUp = () => {
              if (autoRotateSpeedRef.current < 0.14) {
                autoRotateSpeedRef.current = Math.min(
                  0.15,
                  autoRotateSpeedRef.current + 0.012,
                );
